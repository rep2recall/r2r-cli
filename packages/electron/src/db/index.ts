...
export async function initDatabase(filename: string) {
  ...
  if (!MONGO_URI) {
    ...
    CREATE TRIGGER IF NOT EXISTS t_note_attr_ai AFTER INSERT ON note_attr BEGIN
      INSERT INTO note_fts(rowid, note_id, key, data) VALUES (new.id, new.note_id, new.key, new.data);
    END;
    CREATE TRIGGER IF NOT EXISTS t_note_attr_ad AFTER DELETE ON note_attr BEGIN
      INSERT INTO note_fts(note_fts, rowid, note_id, key, data) VALUES ('delete', old.id, old.note_id, old.key, old.data);
    END;
